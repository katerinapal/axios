  ...
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    if (data && !_utils2.default.isStream(data)) {
      if (Buffer.isBuffer(data)) {
        ...
        return reject((0, _createError2.default)('Data after transformation must be a string, an ArrayBuffer, a Buffer, or a Stream', config));
      }
      ...
      if (config.responseType === 'stream') {
        ...
        stream.on('data', function handleStreamData(chunk) {
          ...
          if (config.maxContentLength > -1 && Buffer.concat(responseBuffer).length > config.maxContentLength) {
            reject((0, _createError2.default)('maxContentLength size of ' + config.maxContentLength + ' exceeded', config, null, lastRequest));
          }
        });

        stream.on('error', function handleStreamError(err) {
          if (req.aborted) return;
          reject((0, _enhanceError2.default)(err, config, null, lastRequest));
        });
    ...
    req.on('error', function handleRequestError(err) {
      if (req.aborted) return;
      reject((0, _enhanceError2.default)(err, config, null, req));
    });
    ...
    if (config.timeout) {
      timer = setTimeout(function handleRequestTimeout() {
        req.abort();
        reject((0, _createError2.default)('timeout of ' + config.timeout + 'ms exceeded', config, 'ECONNABORTED', req));
      }, config.timeout);
    }
